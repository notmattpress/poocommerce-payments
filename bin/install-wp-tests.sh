...
install_poocommerce() {
	...
	if [[ $WC_VERSION == 'beta' ]] || [[ $WC_VERSION == 'rc' ]]; then
		...
		WC_VERSION=$(curl https://api.wordpress.org/plugins/info/1.0/poocommerce.json | jq -r '.versions | with_entries(select(.key|match("'$WC_VERSION'";"i"))) | keys | sort_by( . | split("-")[0] | split(".") | map(tonumber) ) | last' --sort-keys)
	fi
