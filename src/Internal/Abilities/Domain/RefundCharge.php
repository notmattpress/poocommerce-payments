...
/**
 * Registers the `poocommerce-payments/refund-charge` ability.
 *
...
class RefundCharge extends AbstractWCPayAbility implements AbilityDefinition {
	...
	public static function get_name(): string {
		return 'poocommerce-payments/refund-charge';
	}
	...
	public static function get_registration_args(): array {
		return [
			'label'               => __( 'Refund a charge', 'poocommerce-payments' ),
			'description'         => __( 'Create a full or partial refund on a Stripe charge. Supply a stable idempotency_key to make retries safe — identical keys return the original refund. Amount is in minor units (e.g. cents).', 'poocommerce-payments' ),
			...
			'input_schema'        => [
				...
				'properties'           => [
					'charge_id'       => [
						'type'        => 'string',
						'pattern'     => '^(ch_|py_)',
						'description' => __( 'Stripe charge ID to refund (ch_… or py_…).', 'poocommerce-payments' ),
					],
					'amount'          => [
						'type'        => 'integer',
						'minimum'     => 1,
						'description' => __( 'Amount to refund in minor units (e.g. cents). Omit for a full refund.', 'poocommerce-payments' ),
					],
					'reason'          => [
						'type'        => 'string',
						'enum'        => [ 'duplicate', 'fraudulent', 'requested_by_customer' ],
						'description' => __( 'Optional refund reason.', 'poocommerce-payments' ),
					],
					'idempotency_key' => [
						'type'        => 'string',
						'description' => __( 'Required. A stable, caller-unique key so duplicate retries dedupe to the original refund instead of creating a second one. Reuse the exact same key when retrying the same logical refund.', 'poocommerce-payments' ),
					],
				],
	...
	public static function execute( $input = null ) {
		if ( ! is_array( $input ) || ! isset( $input['charge_id'] ) || ! is_string( $input['charge_id'] ) || '' === $input['charge_id'] ) {
			return new \WP_Error(
				...
				__( 'A charge_id is required to create a refund.', 'poocommerce-payments' )
			);
		}
		...
		if ( ! isset( $input['idempotency_key'] ) || ! is_string( $input['idempotency_key'] ) || '' === $input['idempotency_key'] ) {
			return new \WP_Error(
				...
				__( 'An idempotency_key is required so refund retries are safe.', 'poocommerce-payments' )
			);
		}
