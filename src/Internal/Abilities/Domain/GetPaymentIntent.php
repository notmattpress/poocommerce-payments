...
/**
 * Registers the `poocommerce-payments/get-payment-intent` ability.
 *
...
class GetPaymentIntent implements AbilityDefinition {
	...
	public static function get_name(): string {
		return 'poocommerce-payments/get-payment-intent';
	}
	...
	public static function get_registration_args(): array {
		return [
			'label'               => __( 'Get payment intent by ID', 'poocommerce-payments' ),
			'description'         => __( 'Look up a single payment intent by Stripe ID (pi_…). Answers \'what is the state of intent pi_X?\' during incident response.', 'poocommerce-payments' ),
	...
	public static function execute( $input = null ) {
		if ( ! is_array( $input ) || empty( $input['payment_intent_id'] ) || ! is_string( $input['payment_intent_id'] ) ) {
			return new \WP_Error(
				...
				__( 'A non-empty `payment_intent_id` is required.', 'poocommerce-payments' )
			);
		}
