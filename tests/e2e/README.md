...
- **Subscriptions specs gate release-time compatibility.** The release process explicitly verifies the test package against the latest PooCommerce Subscriptions. `tests/e2e/specs/subscriptions/**` is the only automated coverage of that surface; treat its specs accordingly.
