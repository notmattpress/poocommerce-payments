...
class WC_Payments_Onboarding_Service_Test extends WCPAY_UnitTestCase {
	...
	public function test_filters_registered_properly() {
		...
		$this->assertNotFalse( has_action( 'poocommerce_poocommerce_payments_updated', [ $this->onboarding_service, 'clear_cached_onboarding_fields_data' ] ) );
	}
	...
	public function test_cleanup_on_account_reset_deletes_store_had_woopayments_options() {
		...
		// Assert.
		$this->assertFalse( get_option( 'poocommerce_admin_pes_incentive_woopayments_store_had_woopayments' ) );
		$this->assertFalse( get_option( 'poocommerce_admin_pes_incentive_woopayments_store_had_woopayments_version' ) );
	}
