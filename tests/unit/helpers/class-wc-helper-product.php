...
class WC_Helper_Product {
	...
	public static function create_attribute( $raw_name = 'size', $terms = [ 'small' ] ) {
		...
		if ( ! $attribute_id ) {
			...
			register_taxonomy(
				$taxonomy_name,
				apply_filters( 'poocommerce_taxonomy_objects_' . $taxonomy_name, [ 'product' ] ), // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
				apply_filters( // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
