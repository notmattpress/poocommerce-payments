<?php
/**
 * Tests for WCPay\Internal\Abilities\Domain\GetDeposits.
 *
...
class GetDepositsTest extends WCPAY_UnitTestCase {
	...
	public function test_name(): void {
		$this->assertSame( 'poocommerce-payments/get-deposits', GetDeposits::get_name() );
	}
	...
	public function test_registration_args_uses_paginated_input_and_envelope_output(): void {
		...
		$this->assertArrayHasKey( 'deposits', $args['output_schema']['properties'] );
		$this->assertSame( 'array', $args['output_schema']['properties']['deposits']['type'] );
	...
	public function test_execute_defaults_page_and_per_page_when_input_is_empty(): void {
		...
		$filter  = function ( $result, $server, $request ) use ( $fixture ) {
			if ( $request->get_route() === '/wc/v3/payments/deposits' ) {
		...
		try {
			$result = GetDeposits::execute( [] );
		} finally {
			remove_filter( 'rest_pre_dispatch', $filter, 10 );
		}

		$this->assertIsArray( $result );
		$this->assertArrayHasKey( 'deposits', $result );
		$this->assertSame( [ [ 'id' => 'po_1' ], [ 'id' => 'po_2' ] ], $result['deposits'] );
