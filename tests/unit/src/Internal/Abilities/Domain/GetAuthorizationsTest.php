...
use WCPay\Internal\Abilities\Domain\GetAuthorizations;

/**
 * @coversDefaultClass \WCPay\Internal\Abilities\Domain\GetAuthorizations
 */
class GetAuthorizationsTest extends WCPAY_UnitTestCase {
	...
	public function test_registration_args_uses_paginated_input_and_envelope_output(): void {
		...
		$this->assertArrayHasKey( 'authorizations', $args['output_schema']['properties'] );
		$this->assertSame( 'array', $args['output_schema']['properties']['authorizations']['type'] );
	...
	public function test_registration_args_includes_sort_properties(): void {
		...
		// Authorizations uses pagination only — no domain-specific filter properties.
		// Sort properties are still present.
	...
	public function test_execute_defaults_page_and_per_page_when_input_is_empty(): void {
		...
		try {
			$result = GetAuthorizations::execute( [] );
		} finally {
		...
		$this->assertSame( [ [ 'id' => 'auth_1' ], [ 'id' => 'auth_2' ] ], $result['authorizations'] );
