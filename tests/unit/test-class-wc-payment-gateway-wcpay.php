...
class WC_Payment_Gateway_WCPay_Test extends WCPAY_UnitTestCase {
	...
	public function tear_down() {
		...
		$payment_method_keys = [
			'payment_method',
			'wc-poocommerce_payments-payment-token',
	...
	public function test_maybe_process_upe_redirect_ignores_create_account_form_post() {
		...
		add_filter( 'poocommerce_is_order_received_page', '__return_true' );
		...
		$_GET['wc_payment_method']            = 'poocommerce_payments';
		...
		remove_filter( 'poocommerce_is_order_received_page', '__return_true' );
		unset(
	...
	public function payment_method_title_provider() {
		return [
			'Visa credit card'       => [
				...
				'expected_gateway' => 'poocommerce_payments',
			],
			'Visa debit card'        => [
				'payment_details'  => [
					'type' => 'card',
			...
			'Bancontact'             => [
				...
				'expected_gateway' => 'poocommerce_payments_bancontact',
			],
			'EPS'                    => [
				'payment_details'  => [ 'type' => 'eps' ],
				'expected_title'   => 'EPS',
				'expected_gateway' => 'poocommerce_payments_eps',
			],
			'Przelewy24 (P24)'       => [
				'payment_details'  => [ 'type' => 'p24' ],
				'expected_title'   => 'Przelewy24 (P24)',
				'expected_gateway' => 'poocommerce_payments_p24',
			],
			'iDEAL | Wero'           => [
				'payment_details'  => [ 'type' => 'ideal' ],
				'expected_title'   => 'iDEAL | Wero',
				'expected_gateway' => 'poocommerce_payments_ideal',
			],
			'SEPA Direct Debit'      => [
				'payment_details'  => [ 'type' => 'sepa_debit' ],
				'expected_title'   => 'SEPA Direct Debit',
				'expected_gateway' => 'poocommerce_payments_sepa_debit',
			],
			'BECS Direct Debit'      => [
				'payment_details'  => [ 'type' => 'au_becs_debit' ],
				'expected_title'   => 'BECS Direct Debit',
				'expected_gateway' => 'poocommerce_payments_au_becs_debit',
			],
			'GrabPay'                => [
				'payment_details'  => [ 'type' => 'grabpay' ],
				'expected_title'   => 'GrabPay',
				'expected_gateway' => 'poocommerce_payments_grabpay',
			],
			'Link'                   => [
	...
	public function express_checkout_payment_method_provider() {
		return [
			...
			'Amazon Pay' => [
				...
				'expected_gateway' => 'poocommerce_payments_amazon_pay',
				'payment_details'  => [
					'type' => 'amazon_pay',
				],
			],
	...
	public function test_regular_card_payment_not_detected_as_express() {
		...
		$this->assertEquals( 'poocommerce_payments', $order->get_payment_method() );
	}
	...
	public function test_amazon_pay_propagates_payment_method_and_title_to_subscriptions() {
		...
		$this->assertEquals(
			'poocommerce_payments_amazon_pay',
	...
	public function test_save_payment_method_checkbox_displayed() {
		...
		$this->setOutputCallback(
			function ( $output ) {
				$input_element = preg_match_all( '/.*<input.*id="wc-poocommerce_payments-new-payment-method".*\/>.*/', $output );
	...
	public function test_manage_customer_details_for_order_skips_customer_update_when_changing_subscription_payment_method() {
		...
		do_action( 'shutdown' ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
	...
	public function test_outputs_express_checkout_settings_screen() {
		...
		$this->assertStringMatchesFormat( '%ahref="admin.php?page=wc-settings&#038;tab=checkout&#038;section=poocommerce_payments"%a', $output );
	}
	...
	public function test_mandate_data_is_skipped_for_sepa_without_a_valid_ip() {
		...
		$_POST['wcpay-fraud-prevention-token'] = 'correct-token';
		$_POST['payment_method']               = 'poocommerce_payments_sepa_debit';
		$pi                                    = new Payment_Information( 'pm_test', $order, null, null, null, null, null, '', 'card' );
	...
	public function test_process_payment_for_order_afterpay_clearpay( array $address, array $locale_data, ?string $expected_exception ) {
		$payment_method                              = 'poocommerce_payments_afterpay_clearpay';
	 ...
	 * hooked into a post-payment PooCommerce hook (e.g. Square's inventory sync
	 * on poocommerce_reduce_order_stock) throws an exception that escapes
	...
	public function test_get_recommended_payment_method_no_country_code_provided( $is_provider_connected, $country_code ) {
		...
		add_filter( 'poocommerce_countries_base_country', $filter_callback );
		...
		// Clean up.
		remove_filter( 'poocommerce_countries_base_country', $filter_callback );
	}
	...
	public function test_updating_subscription_for_non_3ds_cards_removes_hook() {
		...
		add_filter(
			'poocommerce_subscriptions_update_payment_via_pay_shortcode',
		...
		$this->assertEquals(
			10,
			has_filter( 'poocommerce_subscriptions_update_payment_via_pay_shortcode', [ $this->card_gateway, 'update_payment_method_for_subscriptions' ] ),
	...
	public function test_has_custom_place_order_button_set_for_express_checkout_when_feature_enabled() {
		// The feature requires PooCommerce 10.6.0+.
		if ( ! defined( 'WC_VERSION' ) || version_compare( WC_VERSION, '10.6.0', '<' ) ) {
			$this->markTestSkipped( 'Test requires PooCommerce 10.6.0+' );
		}
	...
	public function test_ensure_payment_method_token_for_order_reuses_existing_amazon_pay_token() {
		...
		$order->set_payment_method( 'poocommerce_payments_amazon_pay' );
		$order->save();
	...
	public function test_update_order_status_does_not_reduce_stock_when_changing_subscription_payment_method() {
		...
		// PooCommerce Subscriptions is loaded in the CI environment).
		$this->mock_token_service
			->method( 'add_payment_method_to_user' )
			->willReturn( $token );
	...
	public function test_update_order_status_sets_branded_title_before_status_transition() {
		...
		add_action( 'poocommerce_order_status_changed', $capture, 1, 1 );
		...
		try {
			...
			remove_filter( 'wp_doing_ajax', '__return_true' );
			remove_filter( 'wp_die_ajax_handler', [ $this, 'return_ajax_wp_die_handler' ] );
			remove_action( 'poocommerce_order_status_changed', $capture, 1 );
		}
