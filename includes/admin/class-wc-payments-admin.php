...
class WC_Payments_Admin {
	...
	public function add_payments_menu() {
		...
		if ( WC_Payments_Features::is_reports_area_enabled() ) {
			$this->admin_child_pages['wc-payments-reports'] = [
				...
				'title'    => __( 'Reports', 'poocommerce-payments' ),
				'parent'   => 'wc-payments',
	...
	public function register_payments_scripts() {
		...
		wp_set_script_translations( 'WCPAY_WC_PAYMENTS_SETTINGS_SPOTLIGHT', 'poocommerce-payments' );
		...
		wp_set_script_translations( 'WCPAY_REVIEW_PROMPT', 'poocommerce-payments' );
	...
	private function get_js_settings(): array {
		...
		$this->wcpay_js_settings = [
			...
			'formattedStoreAddress'              => WC()->countries->get_formatted_address(
				[
					'address_1' => get_option( 'poocommerce_store_address', '' ),
					'address_2' => get_option( 'poocommerce_store_address_2', '' ),
					'city'      => get_option( 'poocommerce_store_city', '' ),
					'state'     => WC()->countries->get_base_state(),
					'postcode'  => get_option( 'poocommerce_store_postcode', '' ),
					'country'   => WC()->countries->get_base_country(),
				],
				', '
			),
	...
	/**
	 * Enqueue the spotlight promotion script on PooCommerce Payments Settings page.
	 * Only runs on PooCommerce 9.9.2+ (when the new PooCommerce Payments Settings UI was enabled for all stores).
	 */
	public function enqueue_wc_payment_settings_spotlight() {
		// Check for minimum PooCommerce version 9.9.2.
		if ( ! Constants::is_defined( 'WC_VERSION' ) || version_compare( Constants::get_constant( 'WC_VERSION' ), '9.9.2', '<' ) ) {
			return;
		}

		// Only enqueue on the PooCommerce Payments Settings page.
	...
	/**
	 * Inject the container div for the spotlight promotion on PooCommerce payment settings page.
	 * Only runs on PooCommerce 9.9.2+ (when the new PooCommerce Payments Settings UI was enabled for all stores).
	 */
	public function inject_payment_settings_spotlight_container() {
		...
		// Only inject on the PooCommerce Payments settings page.
		if ( ! $this->is_wc_admin_payments_settings_page() ) {
			return;
		}
	...
	/**
	 * Check if we're on the PooCommerce Payments Settings page (general payments tab, no specific section).
	 *
	...
	public function should_show_review_prompt() {
		...
		$dismissed   = (int) get_user_meta( $user_id, 'poocommerce_admin_wc_payments_review_prompt_dismissed', true );
		$maybe_later = (int) get_user_meta( $user_id, 'poocommerce_admin_wc_payments_review_prompt_maybe_later', true );
