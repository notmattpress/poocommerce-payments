			...
			[
				...
				'args'                => [
					...
					'is_express_checkout_in_payment_methods_enabled' => [
						'description'       => __( 'If express checkout methods (Apple Pay, Google Pay, Amazon Pay) should be enabled as options in the payment methods list.', 'poocommerce-payments' ),
						'type'              => 'boolean',
						'validate_callback' => 'rest_validate_request_arg',
					],
	...
	private function build_server_error_response( WP_Error $error ): WP_REST_Response {
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf( __( 'Invalid parameter(s): %s', 'poocommerce-payments' ), $setting_key ),
				'data'    => [
					'status'  => 400,
