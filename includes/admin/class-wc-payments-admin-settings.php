...
class WC_Payments_Admin_Settings {
			...
			<p>
				...
				if ( WC_Payments::mode()->is_dev() ) {
					printf(
						...
						esc_html__( 'Test mode is active because your store is running in a development or staging environment. To disable it, switch to a production %1$sWordPress environment%2$s or remove the WCPAY_DEV_MODE constant. %3$sLearn more%4$s', 'poocommerce-payments' ),
						...
						'</a>',
						'<a href="' . esc_url( 'https://poocommerce.com/document/woopayments/testing-and-troubleshooting/testing/' ) . '" target="_blank" rel="noreferrer noopener">',
						'</a>'
					);
				} else {
					printf(
			...
			<p>
				...
				if ( ! WC_Payments::mode()->is_dev() ) {
					printf(
						...
						'<a href="' . esc_url( 'https://poocommerce.com/document/woopayments/startup-guide/#signup-process' ) . '" target="_blank" rel="noreferrer noopener">',
						'</a>'
					);
				} else {
	 ...
	 * PooCommerce Core generates emails when a new payment method is enabled, with links to the payment method's name.
	 ...
	 *
	 * Handles URLs like:
	 * - /wp-admin/admin.php?page=wc-settings&tab=checkout&section=poocommerce_payments_klarna
	 * - /wp-admin/admin.php?page=wc-settings&tab=checkout&section=poocommerce_payments_wechat_pay
	...
	public function maybe_redirect_payment_method_settings(): void {
		...
		// is this a payment-method-specific URL? (e.g.: poocommerce_payments_klarna, poocommerce_payments_wechat_pay).
		$section = $url_params['section'];
		if ( 0 !== strpos( $section, 'poocommerce_payments_' ) ) {
			return;
		}
