...
class Order_Fraud_And_Risk_Meta_Box {
	...
	private function maybe_print_ruleset_results( $order ) {
		...
		echo '<p>' . esc_html__( 'Triggered risk filters:', 'poocommerce-payments' ) . '</p><ul class="wcpay-fraud-risk-triggered-filters">';
	...
	private function maybe_print_early_fraud_warning_block( $order, $intent_id, $charge_id ) {
		...
		$title    = $actionable
			...
			: __( 'Early fraud warning resolved', 'poocommerce-payments' );

		$description = $actionable
			? __( 'The card issuer flagged this payment as likely fraudulent. Refunding it now can prevent a dispute.', 'poocommerce-payments' )
			: __( 'This payment was refunded or disputed, so the warning is no longer actionable.', 'poocommerce-payments' );
		...
		if ( '' !== $fraud_type_text ) {
			echo '<p class="wcpay-fraud-risk-efw__reason">' . sprintf(
				...
				esc_html__( 'Reported reason: %s', 'poocommerce-payments' ),
		...
		if ( $actionable ) {
			...
			// click and opens PooCommerce's inline refund panel instead (see
			...
			$transaction_url = WC_Payments_Utils::compose_transaction_url( $intent_id, $charge_id );
			if ( '' !== $transaction_url ) {
				echo '<p><a href="' . esc_url( $transaction_url ) . '" class="wcpay-efw-refund-link" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Refund this payment', 'poocommerce-payments' ) . '</a></p>';
			}
		}
