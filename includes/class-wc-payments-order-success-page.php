...
class WC_Payments_Order_Success_Page {
	...
	public function init_hooks() {
		...
		add_filter( 'poocommerce_order_email_verification_required', [ $this, 'maybe_skip_email_verification_after_payment' ], 10, 3 );
	...
	public function show_poocommerce_payments_payment_method_name( $payment_method_title, $abstract_order ) {
		...
		// Link payments go through the card gateway (poocommerce_payments), so the gateway's
	...
	public function replace_order_received_text_for_failed_orders( $text ) {
		...
		$order_id  = apply_filters( 'poocommerce_thankyou_order_id', absint( $wp->query_vars['order-received'] ?? 0 ) ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- PooCommerce core hook, not defined by WooPayments.
		$order_key = apply_filters( 'poocommerce_thankyou_order_key', empty( $_GET['key'] ) ? '' : wc_clean( wp_unslash( $_GET['key'] ) ) ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment, WordPress.Security.NonceVerification.Recommended -- PooCommerce core hook, not defined by WooPayments.
	...
	public function determine_woopay_order_received_verify_known_shoppers( $value ) {
		...
		$order_key = apply_filters( 'poocommerce_thankyou_order_key', empty( $_GET['key'] ) ? '' : wc_clean( wp_unslash( $_GET['key'] ) ) ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- PooCommerce core hook, not defined by WooPayments.
		$order     = wc_get_order( $order_id );
		...
		$verification_grace_period = (int) apply_filters( 'poocommerce_order_email_verification_grace_period', 10 * MINUTE_IN_SECONDS, $order ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- PooCommerce core hook, not defined by WooPayments.
