...
class WC_Payments_Order_Success_Page {
	...
	public function replace_order_received_text_for_failed_orders( $text ) {
		...
		$order_id  = apply_filters( 'poocommerce_thankyou_order_id', absint( $wp->query_vars['order-received'] ?? 0 ) );
		$order_key = apply_filters( 'poocommerce_thankyou_order_key', empty( $_GET['key'] ) ? '' : wc_clean( wp_unslash( $_GET['key'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
