...
class WC_Payments_Order_Success_Page {
				...
				<div class="card-header">
					...
					<div class="payment-details">
						<div class="payment-header">
							...
							printf( esc_html__( 'Order #%s', 'poocommerce-payments' ), $order->get_order_number() );
							?>
						</div>
	...
	public function add_multibanco_payment_instructions_to_order_on_hold_email( $order, $sent_to_admin = false, $plain_text = false, $email = '' ): void {
		...
		if ( $plain_text ) {
			echo "----------------------------------------\n";
			echo __( 'Multibanco Payment instructions', 'poocommerce-payments' ) . "\n\n";
			printf(
			/* translators: %s: expiry date */
				__( 'Expires %s', 'poocommerce-payments' ) . "\n\n",
				$expiry_date
			);
			echo '1. ' . __( 'In your online bank account or from an ATM, choose "Payment and other services".', 'poocommerce-payments' ) . "\n";
			echo '2. ' . __( 'Click "Payments of services/shopping".', 'poocommerce-payments' ) . "\n";
			echo '3. ' . __( 'Enter the entity number, reference number, and amount.', 'poocommerce-payments' ) . "\n\n";
			echo __( 'Entity', 'poocommerce-payments' ) . ': ' . $multibanco_info['entity'] . "\n";
			echo __( 'Reference', 'poocommerce-payments' ) . ': ' . $multibanco_info['reference'] . "\n";
			echo __( 'Amount', 'poocommerce-payments' ) . ': ' . wp_strip_all_tags( $formatted_order_total ) . "\n";
			echo "----------------------------------------\n\n";
		} else {
			?>
