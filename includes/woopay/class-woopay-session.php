...
class WooPay_Session {
	 ...
	 *
	 * Requires manage_poocommerce capability. Always accepts the write,
	...
	public static function ajax_admin_set_woopay_appearance() {
		...
		if ( ! \WC_Payments::get_gateway()->is_woopay_global_theme_support_enabled() ) {
			wp_send_json_error(
				__( 'This action is not available.', 'poocommerce-payments' ),
				403
			);
		}
		...
		if ( empty( $_POST['appearance'] ) || ! is_array( $_POST['appearance'] ) ) {
			wp_send_json_error(
				__( 'Missing or invalid appearance data.', 'poocommerce-payments' ),
				400
			);
		}
		...
		if ( ! \WC_Payments_Styles_Cache::validate_appearance_schema( $appearance ) ) {
			wp_send_json_error(
				__( 'Invalid appearance schema.', 'poocommerce-payments' ),
				400
			);
		}
