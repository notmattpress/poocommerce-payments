...
class WC_Payments {
	...
	public static function init() {
		...
		add_filter( 'poocommerce_email_classes', [ __CLASS__, 'add_post_kyc_activation_email' ], 10 );
		...
		// `poocommerce_blocks_loaded` fires on plugins_loaded@10 (via
		// `poocommerce_loaded`), before this method runs at plugins_loaded@11,
		...
		add_filter( 'poocommerce_printable_order_receipt_css', [ __CLASS__, 'fit_card_brand_icon_for_printable_order_receipt' ], 10, 2 );
		 ...
		 * @see WOOPMNT-6277.
		 */
		add_action( 'poocommerce_poocommerce_payments_updated', [ new \WCPay\Migrations\Payment_Method_Deprecation_Settings_Update( self::get_gateway(), self::get_payment_gateway_map(), 'giropay', '11.0.0' ), 'maybe_migrate' ] );
		add_action( 'poocommerce_poocommerce_payments_updated', [ new \WCPay\Migrations\Payment_Method_Deprecation_Settings_Update( self::get_gateway(), self::get_payment_gateway_map(), 'sofort', '11.1.0' ), 'maybe_migrate' ] );
		...
		add_action( 'poocommerce_poocommerce_payments_updated', [ new \WCPay\Migrations\Delete_Appearance_Transients(), 'maybe_migrate' ] );
		add_action( 'poocommerce_poocommerce_payments_updated', [ new \WCPay\Migrations\Multi_Currency_Cache_Autodetect_Existing_Install(), 'maybe_migrate' ] );
		add_action( 'poocommerce_poocommerce_payments_updated', [ 'WC_Payments_Styles_Cache', 'handle_theme_change' ] );
		...
		// further below, gated on is_admin() && manage_poocommerce.
	...
	/**
	 * Adds the post-KYC activation reminder email to PooCommerce emails.
	 *
	 * @param array $email_classes the email classes.
	 * @return array
	 */
	...
	/**
	 * Fits WCPay card brand artwork in PooCommerce Core printable receipts.
	 *
	...
	public static function init_rest_api() {
		...
		// requests dispatched with an admin screen set (e.g. PooCommerce core fetching
	 ...
	 * PooCommerce from flagging them as "incompatible with block-based checkout".
	 *
	 ...
	 * PHP gateway IDs, so PooCommerce can't match them and shows a false warning.
	 *
	...
	public static function maybe_register_woopay_hooks() {
		...
		if ( $is_woopay_eligible && $is_woopay_enabled ) {
			...
			// Admin-only endpoint: always accepts appearance write (requires manage_poocommerce).
	...
	/**
	 * Bootstraps the Store API cart extension on `poocommerce_blocks_loaded`.
	 *
	 * @return void
	 */
	 ...
	 * @return string
	 */
	public static function filter_poocommerce_form_field_woopay_email( $field, $key, $args, $_unused_value ) {
		$class = $args['class'][0];
	...
	public static function enqueue_cart_scripts() {
		...
		if ( WC_Payments_Utils::is_cart_block() ) {
			// WC Core introduced a change ( https://github.com/poocommerce/poocommerce/pull/48010 ) that delays the registration of `wc-cart-block-frontend`.
