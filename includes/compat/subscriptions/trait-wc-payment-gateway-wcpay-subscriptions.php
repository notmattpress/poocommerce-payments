...
trait WC_Payment_Gateway_WCPay_Subscriptions_Trait {
	...
	private function get_payment_method_title_from_token( $token, $default_title ) {
		...
		if ( $token instanceof WC_Payment_Token_CC ) {
			...
			if ( ! empty( $last4 ) && false === strpos( $default_title, $last4 ) ) {
				...
				return sprintf( __( '%1$s ending in %2$s', 'poocommerce-payments' ), $title, $last4 );
			}
		}
		...
		if ( $token instanceof WC_Payment_Token_WCPay_Amazon_Pay ) {
			...
			if ( ! empty( $email ) && false === strpos( $default_title, $email ) ) {
				...
				return sprintf( __( '%1$s (%2$s)', 'poocommerce-payments' ), $default_title, $email );
			}
		}
		...
		if ( $token instanceof WC_Payment_Token_WCPay_Link ) {
			...
			if ( ! empty( $email ) && false === strpos( $default_title, $email ) ) {
				...
				return sprintf( __( '%1$s (%2$s)', 'poocommerce-payments' ), __( 'Stripe Link', 'poocommerce-payments' ), $email );
			}
		}
	...
	public function maybe_init_subscriptions_hooks() {
		...
		add_filter( 'poocommerce_subscription_payment_method_to_display', [ $this, 'maybe_override_link_subscription_payment_method_display' ], 10, 2 );
					...
					$note                = '' !== $payment_method_name
						? sprintf(
							WC_Payments_Utils::esc_interpolated_html(
								...
								__( 'A payment of %1$s <strong>failed</strong>: the saved payment method <strong>%2$s</strong> can no longer be used. A new payment method is required.', 'poocommerce-payments' ),
								[ 'strong' => '<strong>' ]
							),
							$formatted_amount,
						...
						: sprintf(
							WC_Payments_Utils::esc_interpolated_html(
								...
								__( 'A payment of %1$s <strong>failed</strong>: the saved payment method can no longer be used. A new payment method is required.', 'poocommerce-payments' ),
								[ 'strong' => '<strong>' ]
							),
							$formatted_amount
						);
			...
			if ( is_null( $subscription_token ) || $token->get_id() !== $subscription_token->get_id() ) {
				...
				$subscription->add_order_note(
					sprintf(
						...
						__( 'The saved payment method for this subscription was missing, so WooPayments restored %s from the original order to complete the renewal.', 'poocommerce-payments' ),
						$token->get_display_name()
					)
				);
			}

			$renewal_order->add_order_note( __( 'Recovered missing subscription payment method token from the parent order.', 'poocommerce-payments' ) );
			return $token;
		} catch ( Exception $e ) {
	 ...
	 *
	 * Stripe Link uses the card gateway (poocommerce_payments), so the gateway title is "Card".
		...
		if ( $payment_information->is_using_saved_payment_method() ) {
			...
			try {
				...
				if ( ! empty( $payment_method['link']['email'] ) ) {
					...
					return sprintf( __( '%1$s (%2$s)', 'poocommerce-payments' ), __( 'Stripe Link', 'poocommerce-payments' ), $payment_method['link']['email'] );
				}
			} catch ( Exception $e ) {
				Logger::error( $e );
			}
