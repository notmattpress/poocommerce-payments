...
class WC_Payments_Order_Service {
	...
	private function generate_capture_cancelled_note(): string {
		$note = WC_Payments_Utils::esc_interpolated_html(
			__( 'Payment authorization was successfully <strong>cancelled</strong>.', 'poocommerce-payments' ),
			[ 'strong' => '<strong>' ]
		);
		...
		if ( empty( $refund_reason ) ) {
			$note = sprintf(
				...
				/* translators: %1: the refund amount, %2: WooPayments, %3: ID of the refund */
					__( 'A refund of %1$s was successfully processed using %2$s (<code>%3$s</code>).', 'poocommerce-payments' ),
					[
						'code' => '<code>',
					]
				),
			...
			$note = sprintf(
				...
				/* translators: %1: the successfully charged amount, %2: WooPayments, %3: reason, %4: refund id */
					__( 'A refund of %1$s was successfully processed using %2$s. Reason: %3$s. (<code>%4$s</code>)', 'poocommerce-payments' ),
					[
						'code' => '<code>',
					]
				),
