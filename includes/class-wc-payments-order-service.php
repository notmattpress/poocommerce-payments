...
class WC_Payments_Order_Service {
	...
	/**
	 * Hook ID suffixes for the PooCommerce order emails that receive a persistent "[Test]"
	...
	public function init_hooks(): void {
		...
		foreach ( self::TEST_MODE_INDICATOR_EMAIL_IDS as $email_id ) {
			add_filter( "poocommerce_email_subject_{$email_id}", [ $this, 'maybe_add_test_mode_to_email_subject' ], 10, 2 );
			add_filter( "poocommerce_email_heading_{$email_id}", [ $this, 'maybe_add_test_mode_to_email_heading' ], 10, 2 );
		}
	}
	 ...
	 * in test mode. Hooked to `poocommerce_email_subject_{$email_id}` for the order emails in
	 ...
	 * in test mode. Hooked to `poocommerce_email_heading_{$email_id}` for the order emails in
	 * self::TEST_MODE_INDICATOR_EMAIL_IDS.
	 *
	...
	public function mark_payment_dispute_closed( $order, $charge_id, $status, $dispute_summary = [], $dispute_id = '' ): void {
		...
		if ( 'lost' === $status ) {
			...
			$order->add_order_note(
				__( 'The order was not marked as completed because it has already been fully refunded.', 'poocommerce-payments' )
			);
		} else {
	...
	public function mark_terminal_payment_failed( $order, string $intent_id, string $intent_status, string $charge_id, string $message ) {
		...
		if ( Order_Status::FAILED === $order_status_before_update ) {
			do_action( 'poocommerce_order_status_pending_to_failed_notification', $order->get_id(), $order ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- PooCommerce core hook, not defined by WooPayments.
			do_action( 'poocommerce_order_status_failed_notification', $order->get_id(), $order ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- PooCommerce core hook, not defined by WooPayments.
		}
	}
	...
	private function prepend_test_mode_email_marker( string $text ): string {
		...
		return sprintf( __( '[Test] %s', 'poocommerce-payments' ), $text );
	}
	...
	private function generate_test_mode_payment_success_note( $intent_id, $charge_id, $formatted_amount ) {
		...
		return sprintf(
			WC_Payments_Utils::esc_interpolated_html(
				...
				__( 'A test payment of %1$s was processed using %2$s in <strong>test mode</strong> (<a>%3$s</a>). No real funds were collected.', 'poocommerce-payments' ),
				[
					'strong' => '<strong>',
					'a'      => ! empty( $transaction_url ) ? '<a href="%4$s" target="_blank" rel="noopener noreferrer">' : '<code>',
				]
	...
	private function generate_fraud_held_for_review_note( $order, $intent_id, $charge_id, array $ruleset_results = [] ): string {
		...
		return $this->generate_fraud_note(
			...
			__( '&#x26D4; A payment of %1$s was <strong>held for review</strong> by the following risk filters:<br>%2$s<br><br><a>View more details</a>.', 'poocommerce-payments' ),
	...
	private function generate_fraud_blocked_note( $order, $intent_id, array $ruleset_results = [] ): string {
		...
		return $this->generate_fraud_note(
			...
			__( '&#x1F6AB; A payment of %1$s was <strong>blocked</strong> by the following risk filters:<br>%2$s<br><br><a>View more details</a>.', 'poocommerce-payments' ),
	...
	private function generate_dispute_created_note( $charge_id, $amount, $reason, $due_by, $is_inquiry = false, $dispute_id = '' ) {
		...
		if ( '' !== $dispute_id ) {
			$note .= ' ' . sprintf(
				...
				esc_html__( '(Dispute ID: %s)', 'poocommerce-payments' ),
				esc_html( $dispute_id )
			);
		}
	...
	private function generate_early_fraud_warning_note( $charge_id, $actionable, $fraud_type ) {
		...
		if ( ! $actionable ) {
			return sprintf(
				WC_Payments_Utils::esc_interpolated_html(
					__( 'The early fraud warning received for this payment is no longer actionable, because the payment was refunded or disputed. See <a>payment details</a> for more information.', 'poocommerce-payments' ),
					[
		...
		// order JS can open PooCommerce's inline refund panel in place (see
		// client/order/index.js); the payment details href is its fallback.
		if ( '' === $reason ) {
			return sprintf(
				WC_Payments_Utils::esc_interpolated_html(
					__( 'Payment has received an early fraud warning. <refund>Refunding the payment now</refund> can prevent a dispute. See <a>payment details</a> for more information.', 'poocommerce-payments' ),
					[
		...
		return sprintf(
			WC_Payments_Utils::esc_interpolated_html(
				...
				__( 'Payment has received an early fraud warning with reason "%1$s". <refund>Refunding the payment now</refund> can prevent a dispute. See <a>payment details</a> for more information.', 'poocommerce-payments' ),
				[
	...
	private function is_order_fully_refunded( WC_Order $order ): bool {
		// Two independent clauses: has_status() catches PooCommerce's standard
		// fully-refunded transition, the remaining-amount check catches stores that
		// redirect it via poocommerce_order_fully_refunded_status, plus over-refunds.
