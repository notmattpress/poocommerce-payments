...
class WC_Payments_Status {
		...
		return array_merge(
			...
			[
				...
				'clear_wcpay_styles_cache'             => [
					'name'     => __( 'Clear WooPayments calculated styles', 'poocommerce-payments' ),
					'button'   => __( 'Clear', 'poocommerce-payments' ),
					'desc'     => __( 'This tool will clear the styles cached for the WooPayments gateway UI elements at checkout', 'poocommerce-payments' ),
	...
	public function clear_styles_cache() {
		...
		return __( 'WooPayments styles cleared', 'poocommerce-payments' );
	}
		...
		<tbody>
			...
			<?php if ( $this->http->is_connected() ) : ?>
				...
				if ( $this->gateway->is_connected() ) :
					...
					<tr>
						<td data-export-label="Dev Mode"><?php esc_html_e( 'Dev Mode', 'poocommerce-payments' ); ?>:</td>
						<td class="help"><?php echo wc_help_tip( esc_html__( 'Whether WooPayments is running in dev (sandbox) mode and what triggered it.', 'poocommerce-payments' ) ); /* phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped, WordPress.Security.EscapeOutput.OutputNotEscaped */ ?></td>
						<td>
						<?php
						if ( WC_Payments::mode()->is_dev() ) {
							$triggers = WC_Payments::mode()->get_dev_mode_triggers();
							$label    = __( 'Enabled', 'poocommerce-payments' );
							if ( ! empty( $triggers ) ) {
								$label .= ' (' . implode( ', ', $triggers ) . ')';
							}
							echo esc_html( $label );
						} else {
							esc_html_e( 'Disabled', 'poocommerce-payments' );
						}
						?>
						</td>
					</tr>
...
<?php endif; ?>
					...
					<tr>
						<td data-export-label="Support Phone"><?php esc_html_e( 'Support Phone', 'poocommerce-payments' ); ?>:</td>
						<td class="help"><?php echo wc_help_tip( esc_html__( 'The support phone number set in WooPayments settings. If not set, the settings Save button will be disabled.', 'poocommerce-payments' ) ); /* phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped, WordPress.Security.EscapeOutput.OutputNotEscaped */ ?></td>
						<td>
						<?php
						...
						if ( ! empty( $support_phone ) ) {
							...
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . esc_html__( 'Not set', 'poocommerce-payments' ) . '</mark>';
						}
						?>
						</td>
					</tr>
