...
class WC_Payment_Gateway_WCPay extends WC_Payment_Gateway_CC {
	...
	public function payment_fields() {
		/**
		 * Fires to set the gateway for the UPE payment fields.
		 *
		 * @since 5.5.0
		 *
		 * @param string $payment_type_id The selected Stripe payment type ID.
		 */
		do_action( 'wc_payments_set_gateway', $this->get_selected_stripe_payment_type_id() );
		/**
		 * Fires to render the UPE payment fields on the checkout page.
		 *
		 * @since 5.0.0
		 */
...
<?php else : ?>
		...
		<div <?php echo $should_hide ? 'style="display:none;"' : ''; /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ ?>>
			<p class="form-row poocommerce-SavedPaymentMethods-saveNew">
				...
				<label for="<?php echo esc_attr( $id ); ?>" style="display:inline;">
					<?php
					/**
					 * Filters the text shown next to the "save payment method" checkbox at checkout.
					 *
					 * @since 1.3.0
					 *
					 * @param string $text The save-to-account checkbox label text.
					 */
					echo esc_html( apply_filters( 'wc_payments_save_to_account_text', __( 'Save payment information to my account for future purchases.', 'poocommerce-payments' ) ) );
					?>
				</label>
			</p>
		</div>
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			// poocommerce_reduce_order_stock, poocommerce_payment_complete, or
			// poocommerce_order_status_processing) must not flip the order to
			...
			if ( Intent_Status::SUCCEEDED === $this->order_service->get_intention_status_for_order( $order ) ) {
				$order->add_order_note(
					sprintf(
						...
						__( 'Payment succeeded, but a downstream error occurred during post-payment processing: %s. Order status preserved.', 'poocommerce-payments' ),
			...
			/**
			 * Fires when an order fails during payment processing.
			 *
			 * @since 5.8.0
			 *
			 * @param WC_Order  $order The order that failed.
			 * @param Exception $e     The exception thrown during payment processing.
			 */
			...
			/**
			 * Fires after a payment error so the result can be updated.
			 *
			 * @since 10.4.0
			 *
			 * @param Exception $e     The exception thrown during payment processing.
	...
	public function process_payment_for_order( $cart, $payment_information, $scheduled_subscription_payment = false ) {
		...
		/**
		 * Filters whether the order payment can be confirmed without a payment intent.
		 *
		 * In case amount is 0 and we're not saving the payment method, we won't be using intents and can confirm the order payment.
		 *
		 * @since 4.7.0
		 *
		 * @param bool $confirm_without_payment_intent Whether to confirm without a payment intent.
		 */
		if ( apply_filters( 'wcpay_confirm_without_payment_intent', ! $payment_needed && ! $save_payment_method_to_store ) ) {
			...
			if ( $is_changing_payment_method_for_subscription && $payment_information->is_using_saved_payment_method() ) {
				...
				if ( $payment_token instanceof \WC_Payment_Token_WCPay_Link ) {
					$note = sprintf(
						WC_Payments_Utils::esc_interpolated_html(
							...
							__( 'Payment method is changed to: <strong>Link ending in %1$s</strong>.', 'poocommerce-payments' ),
							[
								'strong' => '<strong>',
							]
						),
				...
				/**
				 * Fires after the payment method for a subscription order has changed.
				 *
				 * @since 3.2.0
				 *
				 * @param WC_Order         $order         The subscription order.
				 * @param WC_Payment_Token $payment_token The new payment token.
				 */
			...
			if ( $payment_information->is_using_saved_payment_method() ) {
				...
				if ( $token_for_title instanceof \WC_Payment_Token_WCPay_Link ) {
					$order->set_payment_method_title( __( 'Link', 'poocommerce-payments' ) );
		...
		if ( $payment_needed ) {
			...
			if ( Intent_Status::REQUIRES_ACTION === $status && $payment_information->is_merchant_initiated() ) {
				/**
				 * Allow 3rd-party to trigger some action if needed when a payment requires action.
				 *
				 * @since 2.6.0
				 *
				 * @param WC_Order $order          The order being processed.
		...
		// Stock reduction is left to PooCommerce core: the status transition inside
		// update_order_status_from_intent() triggers poocommerce_order_status_* and
		// poocommerce_payment_complete, both of which run wc_maybe_reduce_stock_levels
	...
	public function set_payment_method_title_for_order( $order, $payment_method_type, $payment_method_details ) {
		...
		if ( $express_checkout_type ) {
			...
			/**
			 * Filters the suffix appended to the express checkout payment method title.
			 *
			 * @since 3.8.0
			 *
			 * @param string $suffix The payment method title suffix.
			 */
	...
	/**
	 * Ensures a payment method is saved as a PooCommerce token and attached to the order/subscription.
	 *
	 * @param WC_Order     $order             The order.
	...
	public function ensure_payment_method_token_for_order( $order, $payment_method_id, $user = null ) {
		...
		if ( ! $user instanceof WP_User || empty( $user->ID ) ) {
			throw new Exception( __( 'Unable to save payment method for subscription. The order customer could not be found.', 'poocommerce-payments' ) );
		}
	...
	public function capture_charge( $order, $include_level3 = true, $intent_metadata = [] ) {
		...
		try {
			...
			if ( $include_level3 && 'poocommerce_payments_amazon_pay' !== $order->get_payment_method() ) {
	...
	public function update_order_status() {
		...
		try {
			...
			if ( $amount > 0 && ! $is_subscription_payment_method_change ) {
				...
				if ( Intent_Status::SUCCEEDED === $status && ! $order->is_paid() && ! $is_subscription_payment_method_change ) {
					...
					/* translators: %1: the successfully charged amount, %2: WooPayments, %3: transaction ID of the payment */
						__( 'A payment of %1$s was successfully charged using %2$s (%3$s).', 'poocommerce-payments' ),
			...
			// fires WCS' poocommerce_subscriptions_paid_for_failed_renewal_order hook, which runs
			...
			if ( $intent->is_authorized() && $should_save_payment_method && ! empty( $payment_method_id ) ) {
				try {
					...
					if ( $is_recurring_payment ) {
						throw new Exception(
							__( 'Unable to save payment method for subscription. Please try again or use a different payment method.', 'poocommerce-payments' )
						);
					}
				}
			}
			...
			if ( $intent->is_authorized() ) {
				// Stock reduction is left to PooCommerce core (see note in process_payment_for_order()).
				if ( ! $is_subscription_payment_method_change ) {
					WC()->cart->empty_cart();
				}
	...
	public function schedule_order_tracking( $order_id, $order = null ) {
		...
		// which fires poocommerce_update_order and would schedule a spurious update event.
	...
	/**
	 * Runs a callback with the `poocommerce_payment_complete_reduce_order_stock` filter forced to false.
	 *
	 ...
	 * inside PooCommerce core would otherwise trigger `wc_maybe_reduce_stock_levels()` via
	 * `poocommerce_payment_complete` and `poocommerce_order_status_*` hooks.
	 *
	...
	private function with_stock_reduction_disabled( callable $callback ) {
		$filter           = 'poocommerce_payment_complete_reduce_order_stock';
