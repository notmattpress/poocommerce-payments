...
class WooPay_Tracker extends Jetpack_Tracks_Client {
	...
	public function should_enable_tracking( $is_admin_event = false, $track_on_all_stores = false ) {
		 ...
		 *
		 * Allows merchants to disable all shopper tracking. Defaults to the PooCommerce global tracking setting.
		 *
		 * @since 10.4.0
		 *
		 * @param bool $enabled Whether shopper tracking is enabled.
		 */
		if ( ! apply_filters( 'wcpay_shopper_tracking_enabled', 'no' !== get_option( 'poocommerce_allow_tracking', '' ) ) ) {
			return false;
		}
