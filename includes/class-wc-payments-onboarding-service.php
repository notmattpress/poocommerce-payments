...
class WC_Payments_Onboarding_Service {
	...
	public function init_hooks() {
		...
		add_filter( 'wc_payments_get_onboarding_data_args', [ $this, 'add_poocommerce_store_id_to_request' ] );
		add_action(
			'update_option_poocommerce_' . WC_Payment_Gateway_WCPay::GATEWAY_ID . '_settings',
			[ $this, 'maybe_handle_gateway_test_mode_toggle' ],
			10,
			2
		);
		add_action( 'poocommerce_poocommerce_payments_updated', [ $this, 'clear_cached_onboarding_fields_data' ] );
	}
	...
	public function cleanup_on_account_reset() {
		...
		// Deleting is safe even though PooCommerce core shares these options: if an
	...
	/**
	 * Hook handler for `update_option_poocommerce_<gateway_id>_settings`. Keeps
	...
	/**
	 * Add the PooCommerce store ID to outgoing onboarding request args.
	 *
	 * @param array $args The request args.
	 *
	 * @return array
	 */
	public function add_poocommerce_store_id_to_request( array $args ): array {
		$store_id_key                 = ( class_exists( '\WC_Install' ) && defined( '\WC_Install::STORE_ID_OPTION' ) )
			? \WC_Install::STORE_ID_OPTION
			: 'poocommerce_store_id';
		$args['poocommerce_store_id'] = get_option( $store_id_key, '' );
		return $args;
	}
