...
class WC_Payments_Explicit_Price_Formatter {
	...
	public static function init() {
		add_filter( 'poocommerce_cart_total', [ __CLASS__, 'get_explicit_price' ], 100 );
		add_filter( 'poocommerce_get_formatted_order_total', [ __CLASS__, 'get_explicit_price' ], 100, 2 );
		add_action( 'poocommerce_admin_order_totals_after_tax', [ __CLASS__, 'register_formatted_poocommerce_price_filter' ] );
		add_action( 'poocommerce_admin_order_totals_after_total', [ __CLASS__, 'unregister_formatted_poocommerce_price_filter' ] );
	}
	 ...
	 * So, we need to add a filter to formatted_poocommerce_price. We use specific actions
	 * to register and unregister the filter, so that only the appropriate prices are affected.
	 */
	public static function register_formatted_poocommerce_price_filter() {
	 ...
	 * to register and unregister the filter, so that only the appropriate prices are affected.
	 */
	public static function unregister_formatted_poocommerce_price_filter() {
